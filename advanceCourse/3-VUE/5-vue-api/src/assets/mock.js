...
export default {
  ...
  effectScatterTooltip: {
    xAxis: {
      type: 'category',
      data: ['郭菊锋', '小石头', 'xing.org1^', 'hah', 'Fri', 'Sat', 'Sun'],
    },
    yAxis: {
      type: 'value',
    },
  ...
  effectScatterDataView: {
      ...
      feature: {
        ...
        dataView: {
          ...
          optionToContent: function (opt) {
            ...
            table += '</tr>'
            // 这里还需要tbody方便改样式
            let errorData = series.filter((col) => {
              // 列数遍历
              return col.type === 'effectScatter'
            })
            axisData.forEach((row, i) => {
              // 行数遍历
              table += '<tr>'
              table += `<td>${axisData[i]}</td>`
              series.forEach((col, colNum) => {
                // 列数遍历
                if (col.type === 'line') {
                  // 正常表
                  table += `
    ...
    series: [
      {
        name: '哈哈哈',
        data: [820, 932, 901, 934, null, 1330, 1320],
        type: 'line',
        showSymbol: false,
      },
      {
